...
// Force immediate activation — without this the SW sits in "waiting"
// forever if any existing client tab is open (classic SW update deadlock).
self.addEventListener('install', () => self.skipWaiting());
self.addEventListener('activate', e => e.waitUntil(self.clients.claim()));

// Wire up the UV fetch handler
const uvSW = new UVServiceWorker();
self.addEventListener('fetch', e => {
  if (uvSW.route(e)) {
    e.respondWith(uvSW.fetch(e));
  }
});